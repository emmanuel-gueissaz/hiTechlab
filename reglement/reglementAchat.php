                ...
                <div style="text-align: center">
                    ...
                    <div class="nouveauReglement" >
                        ...
                        <div  class="AjoutReglement slideBar" id="AjouterAvoir" >
                            ...
                            try {
                                $id = $_GET['id'];
                                $requete = "select email from achat where id_achat = $id; ";
                                $requete = $conn->prepare($requete);
                                $requete->execute();
                                $ligne = $requete->fetch();
                                $email = $ligne['email'];
...
and creer.datee::date >= CURRENT_DATE - 31;";
                                $requete = $conn->prepare($requete);
                                $requete->execute();
                                $ligne = $requete->fetch();
                    ...
                    <div style="text-align: right;">
                        <div  class="reste">
                            ...
                            <?php
                            $id = $_GET['id'];
                            $requete = "select * from calculResteAchat ($id);";
                            $requete = $conn->prepare($requete);
                            $requete->execute();
                            $ligne = $requete->fetch();
                            $reste = $ligne['calculresteachat'];
                            echo $reste . ' €';
            ...
            $id = $_GET['id'];
            $t = "'";
            $requete = "select client.email, nom, prenom from achat
                        inner join client ON client.email = achat.email
                        where achat.id_achat = $id";
            $requete = $conn->prepare($requete);
            $requete->execute();
            $ligne = $requete->fetch();
            $email = $ligne['email'];
            $nom = $ligne['nom'];
            $prenom = $ligne['prenom'];
            $idCrypt = openssl_encrypt($id, "AES-128-ECB", 'lEdEvis26300aBz');
            ...
            $html = '<html><head>'
                    ...
                    . 'Vous venez de faire un règlement, vous pouvez consulter vos règlements. '
                    . '     <div style="text-align:center">  <a   href="http://localhost:8080/hitechlab/partieclient/achat/achat.php?id=' . $idCrypt . '" style="               
 ...
 >Règlements</a></div></div>'
            ...
            if (isset($_POST['modfierMontant'])) {
                ...
                try {
                    ...
                    $update->execute();
                    echo "<script> alert_info_redirect('Règlement modifié', 'success','/hitechlab/reglement/reglementAchat.php?id=$id')</script>";
                } catch (Exception $ex) {
            ...
            if (isset($_POST['utiliserAvoir'])) {
                ...
                try {
                    $insert = "select * from utiliseravoirachat($montant::real, $id);";
                    ...
                    $mail->Subject = 'Reglement Hi Tech lab';
                    $mail->Body = $html;
                    ...
                    echo "<script> alert_info_redirect('Règlement ajouté', 'success','/hitechlab/reglement/ReglementAchat.php?id=$id')</script>";
                } catch (Exception $ex) {
                    //echo "<script> alert_info('erreur', 'error')</script>";
                    echo $ex;
                }
            ...
            if (isset($_POST['valideAjoutReg'])) {
                ...
                $montant = $_POST['montant'];
                try {
                    $insert = "select * from lereglementAchat($reg,'$libelle',$montant,$id_ach);";
                    $insert = $conn->prepare($insert);
                    $insert->execute();
