...
<div style="text-align: center;">
    <form method="POST" class="leMotDePasse" id="leMotDePasse">
        ...
        <h4 class="titreAjout" > Nouvelle couleur </h4> 
        <h4 class="labelAjout" > Nom: </h4> 
        <input class=" inputMdp form-control" type="text"  name="couleur" required=""/><br>
     
       
        <input type="button"  class="btn btn-outline-danger btn-lg inputMdp" value="annuler" onclick="history.back()"/>
        <input type="submit" name="ajoutercouleur" class="btn btn-outline-primary btn-lg inputMdp" value="ajouter" />
...
if(isset($_POST['ajoutercouleur'])){
    ...
    try {
    $insert = "insert into couleur (lib_couleur) values ('$nom')";
    $requete = $conn -> prepare($insert);
    $requete -> execute();
     echo '<script> alert_info("Couleur ajouté","success");    setTimeout(function(){ history.go(-2); }, 1500); </script>';
    } catch (Exception $ex) {
