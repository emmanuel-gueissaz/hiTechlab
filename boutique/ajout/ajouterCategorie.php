...
<div style="text-align: center;">
    <form method="POST" class="leMotDePasse" id="leMotDePasse">
        ...
        <h4 class="titreAjout" > Nouvelle catégorie </h4> 
        ...
        <input type="submit" name="ajoutercateg" class="btn btn-outline-primary btn-lg inputMdp" value="ajouter" />
...
if(isset($_POST['ajoutercateg'])){
    ...
    try {
    $insert = "insert into categorie (id_categ) values ('$nom')";
    $requete = $conn -> prepare($insert);
    $requete -> execute();
     echo '<script> alert_info("catégorie ajouté","success");    setTimeout(function(){ history.go(-2); }, 1500); </script>';
    } catch (Exception $ex) {
